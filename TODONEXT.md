...
- [ ] **既有項複驗**（如尚未確認）：新上市股息年化提示（查 00961）、圖表 tooltip 日期

### 2. 待決小項
- [ ] **My Stocks 分頁自動刷新**：目前用 `IndexedStack` 保活，從首頁搜尋加入後切到「我的股票」需手動按 🔄 才更新。是否要做「切到該分頁時自動刷新」？（已向使用者提出，待回覆）

### 3. 雙殖利率（已定案，待實作）
> 2 年平均在股息結構性走高時會低估現況；改為詳情頁**並列兩個殖利率**。
> **排序與警示仍只用保守的 2 年平均（baseline），雙指標僅詳情頁顯示。**

顯示規則（依 `Listing_Months`）：

| 上市時間 | 指標①「近一年殖利率」 | 指標②（標籤隨情況） | 提示 |
|---|---|---|---|
| ≥ 2 年 | 近12月股息 / 股價 | 「2年平均殖利率」 | — |
| 1–2 年 | ✅ 顯示 | 「上市以來年化」 | 未滿2年，年化估算 |
| < 1 年 | 不顯示 | 「上市以來年化（涵蓋 X 月）」 | 🔴 未滿1年，資料極有限僅供參考 |
| 無配息 | — | — | 尚無配息紀錄（顯「—」非 0%）|

實作範圍（4 層 + 重跑 ETL）：
- [ ] `Stock_Master` 新增 `Dividend_1Y`（近12月現金股利）
- [ ] ETL + 後端 `_fetch_and_upsert` 計算並寫入 `Dividend_1Y`；回應加 `dividend_1y` / `yield_1y`
- [ ] 前端 `Stock` model + 詳情頁數據卡改雙欄、依 `listingMonths` 切換標籤與提示
- [ ] 重跑 ETL 回填
